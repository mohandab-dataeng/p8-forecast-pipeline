{% macro stg_macro_weather(relation, station_id) %}
...
cleaned AS (
    SELECT
        ...
        ROUND(CAST(NULLIF(regexp_replace("Precip__Accum_", '[^0-9.-]', '', 'g'), '') AS NUMERIC) * 25.4, 2) AS precipitation_cumul_mm,
